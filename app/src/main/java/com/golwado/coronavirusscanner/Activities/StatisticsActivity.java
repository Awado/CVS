...
public class StatisticsActivity extends AppCompatActivity {
    ...
    private TableRow makeTableRow(TableRowData tableRowData){
        ...
        for(int i = 0; i<columns.length;i++){
            ...
            column.setLayoutParams(new TableRow.LayoutParams((int)((width-arrowPlaceHolderWidth) / columns.length), rowHeight));
            column.setSingleLine(false);
            row.addView(column);
        }
        ImageView arrowIcon = new ImageView(this);
        arrowIcon.setImageResource(Controller.getInstance().getSystemLanguage() == Language.AR ? R.drawable.arrow_left : R.drawable.arrow_right);
    ...
    private void setUpCountriesTable(String searchInput){
        ...
        for(int i = 0; i < columns.length ; i++){
            ...
            int left = !Controller.getInstance().getSystemLanguage().equals(Language.AR) ? (column.getTag().toString().equals(sortedBy) ?
                    (isDesc ? R.drawable.arrow_down : R.drawable.arrow_up) : 0) : 0;
